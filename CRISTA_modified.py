...
if __name__ == '__main__':
	...
	for i, row in input.iterrows():  # this FOR loop is a modification so that CRISTA can run once over a file of sgRNAs
		sgRNA_seq = input.loc[i, 'protospacer']
		full_dna_seq = input.loc[i, 'upstream'][-3:] + input.loc[i, 'protospacer'] + input.loc[i, 'PAM'] + input.loc[i, 'downstream'][:3]
		...
		try:
			...
			# print(parser.parse_args(['-h'])) --> original version
			exit()

		sgRNA_seq = sgRNA_seq.upper() + "NGG"
		full_dna_seq = full_dna_seq.upper()

		# print("Running CRISTA") --> original version
		### align_sequences
		...
		# print("CRISTA predicted score:", prediction[0])  --> original version

		input.loc[i, "CRISTA_prediction"] = prediction[0]
	input.to_csv(output_file)
